...
import { TrendingUp, Plus, Search, Filter, Trash2, Edit3, X, Loader2, Wallet, Building2, Globe, Calendar, Tag, FileText, CheckCircle2 } from 'lucide-react';
import { useAuth } from '../hooks/useAuth';
import { translations } from '../translations';
import { financeService } from '../services/financeService';
import { Income, BankAccount, Category } from '../types';
    ...
    <div className="w-full pb-24 px-4 sm:px-10 lg:px-16">
      ...
      <div className="mb-8">
        ...
        >
          ...
          <div>
            <h1 className="text-2xl font-black text-slate-900 tracking-tight">Create Income</h1>
                      ...
                      <button
                        ...
                        className={cn(
                          ...
                          formData.paymentMethod === method 
                            ? "bg-emerald-500 border-emerald-400 text-white shadow-lg shadow-emerald-100 scale-[1.02]" 
                ...
                <div className="space-y-10 min-h-[100px]">
                  {formData.paymentMethod === 'Bank' && (
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="relative group">
                      <label className="absolute -top-2.5 left-4 px-2 bg-white text-slate-400 font-bold text-xs z-10 transition-colors group-focus-within:text-emerald-500">
                        Select Bank
                      </label>
                      <select 
                        required
                        className="w-full px-4 py-4 bg-white border border-slate-400 rounded-lg focus:border-emerald-500 outline-none text-black font-black transition-all appearance-none cursor-pointer text-base font-sans"
                  ...
                  {formData.paymentMethod === 'Other' && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-10 p-6 bg-slate-50 border-2 border-slate-100 rounded-[2rem]">
                      <div className="relative group">
                        ...
                        <select 
                          required
                          className="w-full px-4 py-4 bg-white border border-slate-400 rounded-lg focus:border-emerald-500 outline-none text-black font-black transition-all appearance-none cursor-pointer text-base font-sans shadow-sm"
                  ...
                  {(formData.paymentMethod === 'Cash' || formData.accountId) && (
                    <motion.div 
                      ...
                      className="p-6 bg-emerald-50 border-2 border-emerald-100 rounded-3xl flex items-center justify-between shadow-sm relative overflow-hidden group"
                    >
                      <div className="absolute top-0 right-0 w-32 h-32 bg-emerald-500/[0.05] blur-3xl -mr-16 -mt-16 group-hover:bg-emerald-500/[0.08] transition-colors" />
                      <div className="space-y-1 relative z-10">
                        <p className="text-[11px] font-bold text-emerald-600">Current Balance</p>
                        <p className="text-3xl font-black text-slate-900 tracking-tighter">
                          {selectedAccountBalance().toLocaleString()}
                        </p>
                      </div>
                      <div className="p-3 bg-white/50 text-emerald-600 rounded-xl border border-emerald-100/50 relative z-10 shadow-sm">
                        <TrendingUp size={24} />
                      </div>
                    </motion.div>
                  )}
              ...
              <div className="space-y-10">
                ...
                <div className="pt-4 relative z-10">
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className="w-full py-4 bg-emerald-600 text-white rounded-xl font-bold hover:bg-emerald-700 transition-all flex items-center justify-center gap-3 text-sm active:scale-[0.98] disabled:opacity-50 whitespace-nowrap"
                  >
  ...
  return (
    <div className="relative group w-full">
      ...
      <input
        ...
        className="w-full px-4 py-4 bg-white border border-slate-400 rounded-lg outline-none font-black text-black transition-all focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 text-base shadow-sm placeholder:text-slate-300"
      />
    </div>
  );
}
